...
const defaultData = {
  ...
  projects: [
    { title: 'FinTech Mobile App', description: 'React Native app with AI-powered financial insights.', tags: ['React Native', 'Node.js'], imageContent: null },
    { title: 'Data Visualization Platform', description: 'Interactive dashboard for complex data analysis.', tags: ['D3.js', 'Python'], imageContent: null },
    { title: '3D Portfolio Site', description: 'Immersive WebGL experience with 3D elements.', tags: ['Three.js', 'WebGL'], imageContent: null }
  ],
  ...
  return (
    <div className="bg-[#050505] text-white min-h-screen font-sans selection:bg-white/10">
      ...
      <div className="relative z-10">
            ...
            <div className="max-w-7xl mx-auto flex justify-between items-center">
                <div className="flex items-center space-x-3">
                    <div className="w-10 h-10 rounded-xl bg-white/5 backdrop-blur-xl border border-white/10 flex items-center justify-center">
                        <span className="text-sm font-bold text-white tracking-tighter">{logo.initials}</span>
                    </div>
                    <span className="text-lg font-medium text-white tracking-tight">{logo.name}</span>
                </div>
                <div className="hidden md:flex items-center space-x-10">
                    {navLinks.map(link => (
                        <a key={link.label} href={link.href} className="text-white/50 hover:text-white transition-all duration-300 text-sm font-medium">{link.label}</a>
                    ))}
                </div>
                <button 
        ...
        <main id="install"  className="w-full flex flex-col items-center justify-center px-6 pt-32">
        ...
        <section id="projects" className="w-full flex flex-col items-center justify-center px-6 py-20">
                    ...
                    {projects.map((project, index) => (
                        ...
                        >
                            <div className="aspect-video rounded-2xl bg-white/5 mb-6 flex items-center justify-center overflow-hidden border border-white/5 group-hover:border-white/10 transition-colors">
                              {project.imageContent || <div className="text-4xl opacity-20 group-hover:opacity-40 transition-opacity duration-500">✨</div>}
                            </div>
                            <h3 className="text-xl font-semibold text-white mb-3 tracking-tight">{project.title}</h3>
                            <p className="text-white/50 text-sm leading-relaxed mb-6 font-light">{project.description}</p>
                            <div className="flex flex-wrap gap-2">
                                {project.tags.map(tag => (
                                    <span key={tag} className="px-3 py-1 rounded-full bg-white/5 border border-white/10 text-[10px] uppercase tracking-widest font-bold text-white/40 group-hover:text-white/60 transition-colors">
                                      {tag}
                                    </span>
                                ))}
                            </div>
        ...
        <section id="skills" className="w-full flex flex-col items-center justify-center px-6 py-20">
                <div className="flex flex-col sm:flex-row justify-center items-center gap-16 md:gap-24 text-center pb-20">
                    {stats.map((stat, index) => (
                        <React.Fragment key={stat.label}>
                            <div className="group">
                                <div className="text-4xl md:text-5xl font-medium text-white mb-2 tracking-tighter group-hover:scale-110 transition-transform duration-500">{stat.value}</div>
